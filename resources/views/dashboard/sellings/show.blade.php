                            ...
                            <form method="post"
                                ...
                                @if ($selling->sellingDetails->count()>0)
                                    <button id="payment-button" type="submit" class="btn btn-lg btn-info btn-block">
                                        ...
                                        <span
                                            id="payment-button-amount">PAYER ({{number_format($selling->price())}} {{auth()->user()->currency()}})</span>
                        ...
                        <form action="{{route("dashboard.selling_details.store")}}" method="POST" class="form-horizontal">
                            ...
                            <div class="row form-group">
                                <div class="col col-md-3">
                                    <label for="email-input" class=" form-control-label">{{__('Article')}}</label>
                                </div>
                            ...
                            <div hidden class="row form-group">
                                ...
                                <div class="col col-md-3">
                                    <label for="email-input" class=" form-control-label">{{__('')}}</label>
                                </div>
                                <div class="col-12 col-md-9">
                                    <select name="article" id="articles" class="form-control">
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="col-12 col-md-9">
                                    <div class="row">
                                        <div class="col-8 col-xs-8">
                                            <input type="hidden" id="iprice" name="iprice" value="dfvghj">
...
@push('scripts')
    ...
    <script>
        ...
        function loadPrice() {
            var value = $("#items").val();
            $.get("{{url('/api/v1/items')}}/" + value, function (data, status) {
                ...
                if (data != null) {
                    $("#price").val(data.selling_price);
                    $("#iprice").val(data.selling_price);
                    // $("#stock1").text(data.qty + ' ' + data.unit.name);
                    // $("#stock2").val(data.qty);
                } else {
                    $("#price").val("");
                    $("#iprice").val("");
        ...
        function loadArticles() {
            ...
            $.get("{{url('/api/articles/item')}}/" + value, function (data, status) {
                ...
                for (let i = 0; i < data.length; i++) {
                    item = data[i];
                    row += '<option value="' + item.id + '">' + item.color.name + ' ' + item.size.name + '</option>';
